...
export const problems: PracticeProblem[] = [
  {
    ...
    parts: [
      {
        prompt: "A teammate proposes a plain hash set of all words, and on each keystroke scanning every word to test the prefix. Explain why this is a poor fit, and propose a data structure that makes prefix queries efficient. Give the time cost of one lookup in terms of the prefix length and the number of matches.",
  ...
  },
  {
    ...
    parts: [
      {
        prompt: "For the deduplication itself, compare using a hash set versus a balanced binary search tree to track which ids have been seen. Give the average and worst-case cost per event for each, and explain what causes the worst case for the hash set.",
      ...
      {
        prompt: "Now the constraints change. The admin range query (list all seen ids between A and B in sorted order) becomes the dominant, latency-critical operation, run far more often than ingestion. Does your choice flip, and why? What hybrid would you consider?",
  ...
  },
  {
    ...
    parts: [
      {
        prompt: "Explain the lower-bound argument for why any comparison-based sort must take at least O(n log n) comparisons in the worst case. Then explain why an algorithm like merge sort actually achieves O(n log n) by reasoning about its recurrence.",
      ...
      {
        prompt: "Now the constraints change. The keys are integers known to lie in the range 0 to 1000, and the array can be huge. Can you now sort in O(n), and does that contradict the lower bound? Explain precisely.",
  ...
  },
  {
    ...
    parts: [
      {
        prompt: "Explain the most likely cause of the production crash, in terms of what recursion consumes that iteration does not. Then describe how to convert the recursive DFS to an iterative version that avoids the failure, and state what space it uses.",
      ...
      {
        prompt: "Now the constraints change. A colleague says the real fix is just to enable tail-call optimization so the recursion uses constant stack space. Is that argument valid for this traversal? Explain when tail-call optimization can and cannot save a recursive algorithm.",
  ...
  },
  {
    ...
    parts: [
      {
        prompt: "Explain why, despite each resize costing O(n), the amortized cost of an append is O(1). Use the doubling strategy in your argument and show roughly why the total copy work across n appends is bounded by a constant times n.",
      ...
      {
        prompt: "Now the constraints change. A junior engineer suggests growing by a fixed amount (add 100 slots each time) instead of doubling, to save memory. Analyze the amortized append cost under fixed-increment growth, and explain the general principle about which growth factors preserve O(1) amortized append.",
  ...
  },
  {
    ...
    parts: [
      {
        prompt: "State the time and space complexity of the nested-loop approach, then describe a hash-based approach that reduces the time complexity. Give the time and space of the hash approach and explain the tradeoff being made.",
      ...
      {
        prompt: "Now the constraints change. Memory is extremely constrained so you cannot afford the O(n) hash table, but you are allowed to modify the array and it turns out you can sort it. Describe an approach that uses O(1) extra space (beyond sorting) and beats O(n^2), and explain its complexity and why it works.",
