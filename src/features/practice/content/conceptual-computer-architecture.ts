...
export const problems: PracticeProblem[] = [
  {
    ...
    parts: [
      {
        prompt: "Part 1: Explain mechanically why the column-major traversal is so much slower, even though both versions touch exactly the same data the same number of times.",
      ...
      {
        prompt: "Part 2: Now the constraints change - you genuinely must process the data in column order (the algorithm requires it) and the matrix is far larger than the last-level cache. What architectural or software techniques would you use to recover performance, and what is the limiting factor you cannot escape?",
  ...
  },
  {
    ...
    parts: [
      {
        prompt: "Part 1: Explain what kind of hazard this is, why forwarding (bypassing) does not fully eliminate it for a load followed by a dependent instruction, and what the hardware must do.",
      ...
      {
        prompt: "Part 2: Now the constraints change - you are designing the ISA and microarchitecture together, and you want to avoid hardware interlocks entirely to simplify the control logic. What are your options, and what do you trade away with each?",
  ...
  },
  {
    ...
    parts: [
      {
        prompt: "Part 1: Explain why branch prediction matters so much in a deep, wide, out-of-order pipeline, and why a near-random branch is so costly relative to a highly predictable one.",
      ...
      {
        prompt: "Part 2: Now the constraints change - assume the branch truly cannot be predicted (the data is random). What techniques at the algorithm, compiler, or hardware level can reduce or eliminate the misprediction cost, and what is the downside of each?",
  ...
  },
  {
    ...
    parts: [
      {
        prompt: "Part 1: Using Amdahl's law reasoning, explain why adding cores gives diminishing returns. State the relationship plainly and identify what fundamentally caps the speedup.",
      ...
      {
        prompt: "Part 2: Now the constraints change - the customer says they will keep buying bigger machines but will also feed in proportionally larger datasets. Does that change the scaling story, and what other real-world overheads beyond the serial fraction limit multicore speedup in practice?",
  ...
  },
  {
    ...
    parts: [
      {
        prompt: "Part 1: Explain the role of virtual memory and the TLB, what a TLB miss actually triggers, and why a workload can have a decent data cache hit rate yet still be bottlenecked on address translation.",
      ...
      {
        prompt: "Part 2: Now the constraints change - you want to reduce the translation overhead without rewriting the application's access pattern. What mechanisms address this, how do they help, and what are the trade-offs?",
  ...
  },
  {
    ...
    parts: [
      {
        prompt: "Part 1: Define latency versus throughput, explain why optimizing one does not automatically optimize the other, and give an architectural example of a design choice that trades one for the other.",
      ...
      {
        prompt: "Part 2: Now the constraints change - the service has a strict per-request tail-latency SLA (for example, 99th percentile must stay under a deadline) AND must handle very high request volume. Which design do you lean toward, and how do you reconcile the throughput-latency tension?",
