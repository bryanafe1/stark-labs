...
export const problems: PracticeProblem[] = [
  {
    ...
    parts: [
      {
        prompt: "Explain why an open-loop heater produces inconsistent results board to board, and what fundamentally changes if you close the loop with a thermocouple feeding back to the controller.",
      ...
      {
        prompt: "Now suppose the thermocouple has a slow response and reads the true board temperature with a 20-second lag. What problems does that sensor lag introduce in the closed loop, and how should it change the way you tune or design the controller?",
  ...
  },
  {
    ...
    parts: [
      {
        prompt: "Describe the distinct role each PID term plays on this altitude loop, and explain specifically which term is responsible for eliminating the steady few-centimeter offset below the commanded height.",
      ...
      {
        prompt: "Now the drone must hold altitude in gusty wind that pushes it down and then suddenly stops. With integral gain raised to kill the offset, the drone overshoots badly when the gust ends. Explain what is happening and how you would address it.",
  ...
  },
  {
    ...
    parts: [
      {
        prompt: "Explain in terms of feedback, phase lag, and stability margins why increasing the loop gain first speeds the response and then drives the system into sustained oscillation.",
      ...
      {
        prompt: "Now you must keep the fast response the high gain gives you but stop the ringing. Without simply lowering the proportional gain, what could you add or change in the controller to restore stability margin, and why does it work?",
  ...
  },
  {
    ...
    parts: [
      {
        prompt: "Explain the mechanism by which closed-loop cruise control rejects a disturbance like a sustained uphill grade, and why a purely open-loop fixed-throttle approach would fail on the same hill.",
      ...
      {
        prompt: "Now imagine the car also has a forward-looking sensor and a map that tells it a steep grade is coming in 100 meters. How would adding feedforward from that grade information change the control, and what are its benefits and risks compared with relying on feedback alone?",
  ...
  },
  {
    ...
    parts: [
      {
        prompt: "Explain how the location of the closed-loop poles maps to the step-response behavior, and contrast the expected response of System A versus System B.",
      ...
      {
        prompt: "Now a design change adds a slow pole to System B, located much closer to the imaginary axis than its existing poles. Explain the concept of dominant poles and how this new slow pole changes the response and your view of the system order.",
  ...
  },
  {
    ...
    parts: [
      {
        prompt: "Explain why a proportional-only controller leaves a constant error on a step target and a growing error on a ramp, and how adding integral action changes the steady-state error to each input.",
      ...
      {
        prompt: "Now the stage must track that constant-velocity ramp with zero following error, but adding more integrators is hurting stability. What approach lets you drive the ramp error to zero without stacking integrators, and what is the tradeoff you are managing?",
