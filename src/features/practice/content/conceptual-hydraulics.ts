...
export const problems: PracticeProblem[] = [
  {
    ...
    parts: [
      {
        prompt: "Explain the difference between inlet control and outlet control for a culvert. What physically governs the flow in each case, and how would you tell from a headwater-versus-discharge perspective which one is acting?",
      ...
      {
        prompt: "Now the constraints change. The downstream channel backs up so the tailwater submerges the culvert outlet, and the road profile cannot be raised. How does submergence shift which control governs, and what design moves actually buy you more capacity now?",
  ...
  },
  {
    ...
    parts: [
      {
        prompt: "Explain what a hydraulic jump is in terms of flow regime and energy. Why does it form, what determines the depth before and after it, and why is it useful in a stilling basin?",
      ...
      {
        prompt: "Now the constraints change. During a drought-season low flow, the tailwater drops well below the design level. What happens to the jump, why is that a problem, and how do basin designs guard against it?",
  ...
  },
  {
    ...
    parts: [
      {
        prompt: "Define the EGL and HGL and explain how they relate to each other and to the pipe. What does the slope of the EGL tell you, and what does it mean if the HGL drops below the pipe elevation?",
      ...
      {
        prompt: "Now the constraints change. You partly close the valve mid-run to throttle flow. Show how the EGL and HGL change through the valve and downstream, and explain what happens to pressure just upstream and just downstream of the valve.",
  ...
  },
  {
    ...
    parts: [
      {
        prompt: "Using Mannings equation Q = (1/n)*A*R^(2/3)*S^(1/2), explain how each term (n, A, R, S) physically affects channel capacity, and why hydraulic radius R = A/P (area over wetted perimeter) rather than just depth or area is the right geometric measure.",
      ...
      {
        prompt: "Now the constraints change. You cannot steepen the channel (grade is fixed) and right-of-way limits how wide you can go. Between lining the channel (lower n) and deepening it, which gives more capacity per unit of effort, and what hydraulic trade-offs do you weigh?",
  ...
  },
  {
    ...
    parts: [
      {
        prompt: "Explain how a weir measures flow: why discharge depends on the head over the crest, and why a rectangular weir follows roughly Q proportional to H^(3/2) while a V-notch follows Q proportional to H^(5/2). What makes one better for low flows?",
      ...
      {
        prompt: "Now the constraints change. The downstream water level rises during high-flow events and submerges the weir crest. What does submergence do to the head-discharge relationship, why does it break the measurement, and how would you redesign to keep accurate readings across the full range?",
  ...
  },
  {
    ...
    parts: [
      {
        prompt: "Explain the principle of reservoir (storage) flood routing. Why is the outflow peak lower than and delayed relative to the inflow peak, and what is the significance of the point where the inflow and outflow hydrographs cross (the moment peak storage occurs)?",
      ...
      {
        prompt: "Now the constraints change. A larger, rarer storm than the design event occurs and the basin nearly fills to its emergency spillway. How does basin behavior change as it fills, why can a basin sized for a small storm fail to control a bigger one, and what design features address this?",
