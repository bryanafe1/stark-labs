...
export const problems: PracticeProblem[] = [
  {
    ...
    parts: [
      {
        prompt: "Compare the two designs in terms of memory footprint, creation cost, communication, and fault isolation. Why is the thread-per-connection model usually cheaper, and what is the key safety risk it introduces that the process-per-connection model avoids?",
  ...
  },
  {
    ...
    parts: [
      {
        prompt: "Explain at the machine-instruction level why updates are lost. Define the critical section here, and explain why a mutex around the increment fixes the result. Why is declaring the counter volatile NOT sufficient?",
  ...
  },
  {
    ...
    parts: [
      {
        prompt: "Explain the specific interleaving that causes the freeze, and map it onto the four Coffman conditions required for deadlock (mutual exclusion, hold-and-wait, no preemption, circular wait). Which condition is the easiest to eliminate here, and how?",
      ...
      {
        prompt: "Now the constraints change — accounts may live on different machines (distributed locks) where a global ordering is hard to enforce and a lock holder can crash mid-transfer. How would you prevent or recover from deadlock now, and what new tradeoffs (vs the single-machine ordering fix) appear?",
  ...
  },
  {
    ...
    parts: [
      {
        prompt: "Compare First-Come-First-Served (FCFS) and Round-Robin (RR) for this workload. Explain the convoy effect and how the choice of RR time quantum trades off interactive responsiveness against context-switch overhead and throughput.",
      ...
      {
        prompt: "Now the constraints change — you adopt a priority scheduler giving interactive jobs high priority and batch jobs low priority. Interactive latency improves, but some batch jobs now never seem to finish. What is happening, and what mechanism fixes it without abandoning priorities?",
  ...
  },
  {
    ...
    parts: [
      {
        prompt: "Explain how virtual memory and demand paging let a process address more memory than physical RAM. Describe what happens on a page fault, the role of the page table and TLB, and why the program slows as it exceeds RAM.",
      ...
      {
        prompt: "Now the constraints change — the slowdown becomes catastrophic: CPU utilization collapses and the disk is saturated even though the program is doing little useful work. Name this phenomenon, explain the mechanism in terms of working sets, and give two ways to address it.",
  ...
  },
  {
    ...
    parts: [
      {
        prompt: "Explain what a context switch actually does and why oversubscribing the cores with CPU-bound threads reduces throughput. Distinguish the direct cost of switching from the indirect cost, and explain which usually dominates.",
