...
export const problems: PracticeProblem[] = [
  {
    ...
    parts: [
      {
        prompt: "Part 1: What is the maximum throughput of this line, which station governs it, and why does adding effort or speeding up any other station not increase output?",
      ...
      {
        prompt: "Part 2: Now you cut station 3 from 60s to 35s with a process change, but you make no other changes. What happens to throughput, and where does the constraint move? What does this teach about repeated improvement?",
  ...
  },
  {
    ...
    parts: [
      {
        prompt: "Part 1: Explain to management why wait times do not grow linearly with utilization, and why running the agent at 95 to 99 percent busy produces disproportionately long ticket queues. What role does variability play?",
      ...
      {
        prompt: "Part 2: Now suppose you cannot reduce utilization, but you could either reduce the variability of ticket handling times or add a second agent (pooling the queue). Which lever helps more, and why?",
  ...
  },
  {
    ...
    parts: [
      {
        prompt: "Part 1: Why does the optimal solution of a linear program land at a corner (vertex) of the feasible region, and what does it mean that machine hours are fully used but labor hours are not? What is the shadow price of each constraint telling you?",
      ...
      {
        prompt: "Part 2: Now a supplier offers to sell you extra machine hours at a fixed price per hour. How do you decide whether to buy, and is the answer the same no matter how many extra hours you buy?",
  ...
  },
  {
    ...
    parts: [
      {
        prompt: "Part 1: Using Littles Law, estimate the average time an order spends in the system, state the law plainly, and explain what assumptions make it valid here.",
      ...
      {
        prompt: "Part 2: Now the team doubles throughput to 200 orders per hour but wants the average order to spend only 2 hours in the building. What does Littles Law require about work in process, and what are the two distinct ways to cut flow time?",
  ...
  },
  {
    ...
    parts: [
      {
        prompt: "Part 1: Describe the core tradeoff that sets the best order quantity (the EOQ logic). Why is ordering in very large batches wasteful, and why is ordering tiny amounts very often also wasteful?",
      ...
      {
        prompt: "Part 2: Now the supplier lead time becomes more variable (sometimes much longer than usual). How does that change your policy, what is safety stock for, and what service-level tradeoff does it create?",
  ...
  },
  {
    ...
    parts: [
      {
        prompt: "Part 1: Explain how large batch sizes inflate flow time and create the feast-or-famine pattern downstream, connecting your answer to work-in-process and queueing behavior.",
      ...
      {
        prompt: "Part 2: Now cutting batch size is only attractive if setup time is also reduced. Why is setup reduction the enabler, and what is the limit, that is, what stops you from driving batches all the way to one unit?",
