...
export const problems: PracticeProblem[] = [
  {
    ...
    parts: [
      {
        prompt: "Part 1: A pure feedback temperature controller already exists but reacts sluggishly to the inlet flow swings. Explain why feedback alone struggles here, and describe how adding feedforward on the measured inlet flow would improve disturbance rejection.",
      ...
      {
        prompt: "Part 2: Now suppose the inlet flow meter is removed and you can no longer measure the disturbance, but the inlet temperature sensor is reliable. How does this change your control strategy, and what are the limits of what you can now achieve?",
  ...
  },
  {
    ...
    parts: [
      {
        prompt: "Part 1: Explain why large dead time is so destabilizing for a feedback loop, and why simply increasing controller gain to speed up the response tends to make things worse rather than better.",
      ...
      {
        prompt: "Part 2: Management wants tighter composition control despite the delay. Describe what a Smith predictor (or dead-time compensator) does, and what conditions must hold for it to actually help instead of hurt.",
  ...
  },
  {
    ...
    parts: [
      {
        prompt: "Part 1: Explain the distinct roles of the proportional, integral, and derivative terms, and from the symptom described, which term is most likely mistuned and in which direction.",
      ...
      {
        prompt: "Part 2: Now suppose this same loop occasionally saturates because the valve hits fully open during large upsets. The operator reports that after such an event the loop overshoots even more wildly than usual on the way back. What phenomenon is this, and how do you prevent it?",
  ...
  },
  {
    ...
    parts: [
      {
        prompt: "Part 1: List the main candidate causes of a self-sustaining oscillation in a single loop, and explain how you would use the character of the oscillation to distinguish a tuning problem from a mechanical valve problem.",
      ...
      {
        prompt: "Part 2: You put the controller in manual and the oscillation stops completely. Then you slowly ramp the controller output by hand and notice the valve position does not move smoothly but jumps in steps. Reinterpret the root cause and explain why retuning would not fix it.",
  ...
  },
  {
    ...
    parts: [
      {
        prompt: "Part 1: Describe how a cascade scheme with a reactor-temperature master and a jacket-flow (or jacket-temperature) slave would be configured, and explain specifically why cascade rejects the oil header pressure disturbance far better than a single loop.",
      ...
      {
        prompt: "Part 2: Now imagine the slave (inner) loop is tuned slower than the master (outer) loop. Explain what goes wrong, and state the general rule for the relative speeds of the two loops and why it must hold.",
  ...
  },
  {
    ...
    parts: [
      {
        prompt: "Part 1: Explain why level control on a surge drum is often deliberately tuned loose (averaging level control) while a temperature loop is usually tuned for tight setpoint tracking. Address the difference in process dynamics and control objectives.",
