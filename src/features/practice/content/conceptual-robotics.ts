...
export const problems: PracticeProblem[] = [
  {
    ...
    parts: [
      {
        prompt: "Part 1: Explain the difference between forward and inverse kinematics for this arm, which one the vision-driven motion actually requires, and why one direction is generally much harder to solve than the other.",
      ...
      {
        prompt: "Part 2: Now suppose the IK solver returns several valid joint configurations for the same target pose. How would you decide which configuration to actually command, and what real-world constraints make this choice matter?",
  ...
  },
  {
    ...
    parts: [
      {
        prompt: "Part 1: Explain why 6 DOF is the baseline number for positioning a rigid tool arbitrarily in space, and what a 7th joint actually buys you for this inspection task.",
      ...
      {
        prompt: "Part 2: Now the task only requires touching points with a symmetric, ball-shaped probe where tool roll about its own axis does not matter. How does that change the effective DOF requirement, and how would you exploit the freed-up axis?",
  ...
  },
  {
    ...
    parts: [
      {
        prompt: "Part 1: Explain what a kinematic singularity is in terms of the Jacobian, and why a singularity produces exactly the symptom described (huge joint speed for small tool speed, loss of a motion direction).",
      ...
      {
        prompt: "Part 2: Now you must run a trajectory whose straight-line path unavoidably passes through that wrist singularity. What practical strategies let you get through it without the violent joint motion, and what is the tradeoff of each?",
  ...
  },
  {
    ...
    parts: [
      {
        prompt: "Part 1: Explain the difference between the reachable workspace and the dexterous workspace, and why a point can be inside the reachable envelope yet still be unserviceable in a required orientation.",
      ...
      {
        prompt: "Part 2: Now the integrator can either tilt the part fixture by 20 degrees or add a rotary positioner (an extra external axis). Explain how each option changes the effective workspace problem and when you would prefer one over the other.",
  ...
  },
  {
    ...
    parts: [
      {
        prompt: "Part 1: Explain the difference between incremental and absolute encoders and why this difference produces the power-cycle behavior described.",
      ...
      {
        prompt: "Part 2: Now consider the second symptom: joint encoders all read their commanded values yet the tool tip is off by a few millimeters. Encoders are not the problem, so what is, and how does this distinguish encoder feedback accuracy from overall positioning accuracy?",
  ...
  },
  {
    ...
    parts: [
      {
        prompt: "Part 1: Explain the tradeoff between payload and reach in arm selection, and why the rated payload typically falls as the arm extends toward maximum reach.",
      ...
      {
        prompt: "Part 2: Now focus on the insertion itself with the chassis shifting slightly each cycle. Would you want a stiffer or a more compliant end-effector or control scheme here, and explain the compliance-versus-stiffness tradeoff for this task.",
