...
export const problems: PracticeProblem[] = [
  {
    ...
    parts: [
      {
        prompt: "Explain why measuring just the impulse response is enough to predict the output for any input, and state exactly what assumptions about the system make this true. What is the mathematical operation that turns the impulse response plus an arbitrary input into the output?",
      ...
      {
        prompt: "Now the constraints change. You discover the preamp has an automatic gain control (AGC) that quietly reduces gain when the input gets loud. Does the single-impulse-response trick still work? Explain what specifically breaks and how you would even know from measurements.",
  ...
  },
  {
    ...
    parts: [
      {
        prompt: "Using the intuition of convolution as a sliding, flip-and-weight operation, explain physically why the output is rounded and stretched. Connect the shape of the RC filter's impulse response to what happens at the rising and falling edges of the pulse.",
      ...
      {
        prompt: "Now the constraints change. Suppose you keep the same input pulse but the pulse width becomes much SHORTER than RC (a brief spike). Describe how the output shape changes and what the output essentially becomes, and explain why in terms of convolution.",
  ...
  },
  {
    ...
    parts: [
      {
        prompt: "Explain why the real 950 Hz tone appears as a phantom 50 Hz tone. State the Nyquist criterion, explain what aliasing is, and show how to predict the aliased frequency from the sample rate.",
      ...
      {
        prompt: "Now the constraints change. The motor tone cannot be removed at the source, and the team proposes simply raising the sample rate to make the problem go away. Explain when raising fs alone is or is not a real fix, and what additional component is the correct defense regardless of sample rate.",
  ...
  },
  {
    ...
    parts: [
      {
        prompt: "Explain what poles and zeros physically mean for a system's behavior, and why a pole in the right half of the s-plane is a serious problem. Connect pole location to the time-domain response (decay vs growth, oscillation).",
  ...
  },
  {
    ...
    parts: [
      {
        prompt: "Explain why a sinusoid (or complex exponential) is an eigenfunction of any LTI system, what the frequency response H(j*omega) represents in magnitude and phase, and how this justifies treating filtering as multiplication in the frequency domain.",
      ...
      {
        prompt: "Now the constraints change. The interviewer points out that a square wave fed into the same equalizer comes out visibly distorted in shape, with rounded or ringing edges, even though you just claimed LTI systems do not change the waveform. Resolve this apparent contradiction.",
  ...
  },
  {
    ...
    parts: [
      {
        prompt: "Explain why you cannot make a signal arbitrarily narrow in both time and frequency simultaneously. Use the time-frequency duality of the Fourier transform to justify the tradeoff, with concrete examples at the two extremes.",
      ...
      {
        prompt: "Now the constraints change. The engineer insists they truly need BOTH fine range (time) resolution AND fine velocity (frequency/Doppler) resolution. Given you cannot beat the tradeoff with a simple pulse, explain the conceptual escape and why it works without violating the principle.",
