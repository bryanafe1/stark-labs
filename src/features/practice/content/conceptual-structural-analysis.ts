...
export const problems: PracticeProblem[] = [
  {
    ...
    parts: [
      {
        prompt: "Trace the load path of the snow weight from the tip of the canopy all the way into the ground, and identify which internal force dominates at the back edge connection.",
      ...
      {
        prompt: "Now the constraints change -- the architect adds a single diagonal tie rod from the canopy tip up to a higher point on the building face. How does the load path and the governing force at the original back connection change?",
  ...
  },
  {
    ...
    parts: [
      {
        prompt: "Classify each design as statically determinate or indeterminate, and explain how that classification changes the way you would solve for the internal forces.",
      ...
      {
        prompt: "Now the constraints change -- a support settles slightly (one end drops 10 mm) and separately the beam is heated uniformly. For each design, does this induce internal stresses, and why?",
  ...
  },
  {
    ...
    parts: [
      {
        prompt: "Explain the structural role of redundancy here and why low stress in members under normal loads is a weak argument for removing them.",
      ...
      {
        prompt: "Now the constraints change -- suppose the truss is left fully intact but is statically determinate by design (no redundant members). What is the consequence if a single diagonal buckles, and how does that differ from an indeterminate version of the same truss?",
  ...
  },
  {
    ...
    parts: [
      {
        prompt: "Under the lateral load, where do the largest moments concentrate and where would you expect plastic hinges to form, ultimately defining the frame's failure mechanism?",
      ...
      {
        prompt: "Now the constraints change -- the column bases are made fully fixed instead of pinned. How does this shift the moment distribution, the likely hinge locations, and the frame's stiffness and ductility?",
  ...
  },
  {
    ...
    parts: [
      {
        prompt: "Using moment distribution intuition, explain qualitatively how the unbalanced moment at the joint gets shared among the members and what physical principle governs the split.",
      ...
      {
        prompt: "Now the constraints change -- one of the three beams has its far end changed from rigidly fixed to a simple (pinned) support. How does that change the distribution at the joint and the carry-over behavior?",
  ...
  },
  {
    ...
    parts: [
      {
        prompt: "Explain why the triangle is stable and rigid while the pin-connected rectangle is not, and connect this to how truss members carry load.",
      ...
      {
        prompt: "Now the constraints change -- two truss designs span the same gap with the same total amount of steel: design A is a shallow (short-depth) truss and design B is a deep (tall) truss. Which deflects less under the same load and why?",
